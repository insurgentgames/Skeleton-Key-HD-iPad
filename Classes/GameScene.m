...
- (id) init {
	if((self=[super init])) {
		...
		achievements = ((SkeletonKeyHDAppDelegate*)([UIApplication sharedApplication].delegate)).achievements;
...
- (void) setCurrentGame {
    ...
    SkeletonKeyHDAppDelegate* delegate = ((SkeletonKeyHDAppDelegate*)([UIApplication sharedApplication].delegate));
...
- (void) restartLevel {
	...
	menu.position = ccp(164, 964);
	[self addChild:menu z:4];
	
	// heads up display
	CCLabelTTF* level = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"%i", [gameData level]] fontName:@"deutsch.ttf" fontSize:54];
	level.position = ccp(384, 969);
	[self addChild:level z:4];
	CCLabelTTF* moves = [CCLabelTTF labelWithString:[NSString stringWithFormat:@"%i", [gameData movesLeft]] fontName:@"deutsch.ttf" fontSize:54];
	moves.position = ccp(604, 969);
...
- (void) onCompletedLevel:(id)sender {
    ...
    levelCompleteMessage.position = ccp(384, 512);
    ...
    moves *= -1;
    if(moves <= 0) {
        levelCompleteLabel = [CCLabelTTF labelWithString: @"Perfect!" fontName:@"gabriola.ttf" fontSize:60];
        [levelCompleteLabel runAction:[CCFadeIn actionWithDuration:0.5]];
        levelCompleteLabel.position = ccp(384, 382);
        ...
        levelCompletePerfect.position = ccp(384, 462);
        ...
        levelCompleteLabel = [CCLabelTTF labelWithString: @"Good Job!" fontName:@"gabriola.ttf" fontSize:55];
        [levelCompleteLabel runAction:[CCFadeIn actionWithDuration:0.5]];
        levelCompleteLabel.position = ccp(384, 492);
        [levelCompleteLayer addChild:levelCompleteLabel];
        ...
        levelCompleteLabel2 = [CCLabelTTF labelWithString: completeText fontName:@"gabriola.ttf" fontSize:55];
        [levelCompleteLabel2 runAction:[CCFadeIn actionWithDuration:0.5]];
        levelCompleteLabel2.position = ccp(384, 422);
        [levelCompleteLayer addChild:levelCompleteLabel2];
        
        levelCompleteLabel3 = [CCLabelTTF labelWithString: @"From Perfect" fontName:@"gabriola.ttf" fontSize:55];
        [levelCompleteLabel3 runAction:[CCFadeIn actionWithDuration:0.5]];
        levelCompleteLabel3.position = ccp(384, 352);
...
- (void) messageDisplay:(int)messageToDisplay {
	...
	switch(messageToDisplay) {
		...
		case GameMessageSwipeScreen:
			...
			spriteMessage.position = ccp(384, 519);
			break;
		case GameMessageNoMovesMenu:
			...
			spriteMessage.position = ccp(384, 574);
			break;
		case GameMessageNoMovesShake:
			...
			spriteMessage.position = ccp(384, 629);
			break;
	}
	if(spriteMessage != nil) {
...
- (void) unlockAchievement:(int)achievementId {
	...
	if([achievements unlock:achievementId]) {
        ...
        achievementBackground.position = ccp(384, 232);
        [achievementLayer addChild:achievementBackground z:0];
        CCLabelTTF* achievementTitle = [CCLabelTTF labelWithString:[achievements getName:achievementId] fontName:@"gabriola.ttf" fontSize:63];
        achievementTitle.color = ccc3(255, 255, 255);
        achievementTitle.position = ccp(494, 134);
        [achievementLayer addChild:achievementTitle z:1];
        CCLabelTTF* achievementDesc = [CCLabelTTF labelWithString:[achievements getDescription:achievementId] fontName:@"gabriola.ttf" fontSize:45];
        achievementDesc.color = ccc3(196, 207, 226);
        achievementDesc.position = ccp(494, 72);
        [achievementLayer addChild:achievementDesc z:1];
        
        // animate it
        achievementLayer.position = ccp(0, -468);
        [achievementLayer runAction:[CCSequence actions:
                                     [CCMoveTo actionWithDuration:1.0 position:ccp(0, 0)],
                                     [CCDelayTime actionWithDuration:2.0],
                                     [CCMoveTo actionWithDuration:1.0 position:ccp(0, -468)],
