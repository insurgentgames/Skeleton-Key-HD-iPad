...
- (id) init {
	if((self=[super init])) {
		...
		for(NSUInteger i=0; i<achievements.numAchievements; i++) {
			...
			achievement_shadow.position = ccp(384+768*page, 197+146*y);
			[pageLayer addChild:achievement_shadow z:2];
			
			// text
			CCLabelTTF* achievment_label_top = [CCLabelTTF labelWithString:topText fontName:@"gabriola.ttf" fontSize:61];
			achievment_label_top.position = ccp(477.5+768*page, 225+146*y);
			[pageLayer addChild:achievment_label_top z:3];
			CCLabelTTF* achievment_label_bottom = [CCLabelTTF labelWithString:bottomText fontName:@"gabriola.ttf" fontSize:46];
			achievment_label_bottom.position = ccp(477.5+768*page, 169+146*y);
			...
			if([achievements.achieved objectAtIndex:i] == [NSNumber numberWithInt:1]) {
				...
				achievement_trophy.position = ccp(162+768*page, 197+146*y);
				...
				achievement_trophy_locked.position = ccp(162+768*page, 197+146*y);
